...
-- Question 3: Which Month & Category had the highest gross profit rate % growth rate MoM?
-- Answer 3: Scotch Whiskey in month 5
...
SELECT 
  ...
  ROUND(gross_profit_percent / LAG(gross_profit_percent, 1) OVER (
    PARTITION BY category_group
    ORDER BY month
  ) - 1, 2) gross_profit_percent_growth_rate

FROM DUBUQUE_CTE

ORDER BY gross_profit_percent_growth_rate DESC
